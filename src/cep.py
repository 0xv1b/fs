...
# xscan_theta = [-23.008359106593396,-20.062554042460626,-20.685005980061668,-17.817791094417586,-27.179657761455807,-22.126972672615643,-22.166268016571046]
# xscan_j0 = [201.05294242176504,30.292548463526956,80.60352080051584,230.51397993483187,214.04398823427226,231.93641657727756,88.89718404682124]
...
# axes[1].axvspan(1.16952, 1.17547, facecolor='blue', alpha=0.5)
# axes[1].axvspan(1.14646, 1.14949, facecolor='red', alpha=0.5)


# axes[0].errorbar(xscan180_keys, xscan180_j0, marker=".", linestyle="--", markersize=8, color="black",xerr=0.0025)
# axes[0].axvspan(1.16752, 1.17649, facecolor='blue', alpha=0.5)
# axes[0].axvspan(1.14451, 1.15051, facecolor='red', alpha=0.5)
...
# Data
xscan_keys = [1.148,1.1529999999999998,1.1579999999999997,1.1629999999999996,1.1679999999999995,1.1729999999999994,1.1779999999999993]
xscan_ipl = [0.11574724965210058,0.12552673884479312,0.1138344625198837,0.14332755563773583,0.10149822657938015,0.11918298218851806,0.11972916302992587]
xscan_theta = [-23.008359106593396,-20.062554042460626,-20.685005980061668,-17.817791094417586,-27.179657761455807,-22.126972672615643,-22.166268016571046]
xscan_j0 = [201.05294242176504,30.292548463526956,80.60352080051584,230.51397993483187,214.04398823427226,231.93641657727756,88.89718404682124]
xscan_j0 = [x/10 for x in xscan_j0]
...
ax1.set_xlabel('x position (mm)', fontsize=18)
ax1.set_ylabel(r'$j_0$ (pA)', fontsize=18)
...
# Photocurrent Plot
ax1.errorbar(xscan180_keys, xscan180_j0, marker=".", linestyle="--", markersize=12, color="black") #, xerr=0.0025)
ax1.axvspan(1.16752, 1.17649, facecolor='blue', alpha=0.5)
ax1.axvspan(1.14451, 1.15051, facecolor='red', alpha=0.5)
ax1.set_ylim(-0.19, 51)

# Theta Plot
ax1.plot(xscan180_keys, [(x % (2*np.pi))*max(xscan180_j0)/(2*np.pi) for x in xscan180_theta], marker="x", linestyle="", markeredgewidth=2, markersize=8, color="red")

plt.savefig("./images/XScan180_Phase.pdf")
plt.tight_layout()
plt.show()

# ---------------------------------------------------------------------------- #
# Plot 2 (for xscan_keys and xscan_j0)                                        #
# ---------------------------------------------------------------------------- #
...
ax2.set_xlim(1.14, 1.18) 
ax2.set_xlabel('x position (mm)', fontsize=18)
ax2.set_ylabel(r'$j_0$ (pA)', fontsize=18)
...
# Create secondary y-axis for theta
period_axis2 = ax2.secondary_yaxis('right', functions=(j2theta, theta2j), color="red")
period_axis2.set_ylabel(r'$\Theta_{CEP,ins}$ (2$\pi$ radians)', fontsize=18)
period_axis2.tick_params(axis='y', direction='in')
period_axis2.tick_params(axis='both', which='major', labelsize=12)
period_axis2.tick_params(axis='both', which='minor', labelsize=12)

# Photocurrent Plot
ax2.errorbar(xscan_keys, xscan_j0, marker=".", linestyle="--", markersize=12, color="black") #, xerr=0.0025)
ax2.axvspan(1.16952, 1.17547, facecolor='blue', alpha=0.5)
ax2.axvspan(1.14646, 1.14949, facecolor='red', alpha=0.5)

# Theta Plot
ax2.plot(xscan_keys, [(x % (2*np.pi))*max(xscan_j0)/(2*np.pi) for x in xscan_theta], marker="x", linestyle="", markeredgewidth=2, markersize=8, color="red")

ax2.set_ylim(-0.12, 23.34)
plt.tight_layout()
plt.savefig("./images/XScan_Phase.pdf")
plt.show()
